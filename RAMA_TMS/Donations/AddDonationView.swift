...
struct AddDonationView: View {
    ...
    @State private var generatedReceiptNumber = ""
    
    @State private var showZelleQR = false
    @FocusState private var focusedField: Field?
    
    // Using consolidated OfflineManager only
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Offline Mode")
                        ...
                        .fontWeight(.semibold)
                        .foregroundColor(.orange)
                    Text("Donation will be saved locally and synced when online")
                        .font(.caption)
        ...
        Button(action: submit) {
            HStack(spacing: 12) {
                if isSubmitting {
                    ...
                    Text(offlineManager.isOnline ? "Save Donation" : "Save Offline")
                        .fontWeight(.semibold)
                        .font(.headline)
                }
            }
    ...
    func submit() {
        ...
        Task {
            do {
                // Check if online using consolidated OfflineManager
                let isOnline = offlineManager.isOnline
                
                if isOnline {
                    // ONLINE: Submit directly to API
                    try await submitOnline(amount: amt)
                } else {
                    // OFFLINE: Save locally using consolidated OfflineManager
                    saveOffline(amount: amt)
                }
            } catch {
                await MainActor.run {
                    withAnimation {
    ...
    private func submitOnline(amount: Double) async throws {
        ...
        do {
            ...
            // If online submission fails and we're actually offline, save locally
            if !offlineManager.isOnline {
                await MainActor.run {
                    saveOffline(amount: amount)
                }
            } else {
                throw error
            }
    ...
    private func saveOffline(amount: Double) {
        ...
        generatedReceiptNumber = donation.receiptNumber
        
        withAnimation {
            statusMessage = """
            Donation saved offline!
            Receipt: \(generatedReceiptNumber)
